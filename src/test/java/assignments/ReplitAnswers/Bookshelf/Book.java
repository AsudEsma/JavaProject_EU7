...
public class Book {
    ...
    public void setAuthor(String author) {
        if(author.isEmpty()) {
    		...
    		for(String each : arr) {
    			...
    			name += capitalize(each) + " "; 
    		}
    		
    		this.author = name.trim();
    ...
    public void setTitle(String title) {
    	...
    	if(title.isEmpty()){
    		...
    		this.title = capitalize(title);			// nArniA  -> Narnia
    		
    	}  else {
    		
    		String[] arr = title.split(" ");  // THE, Chronicles, OF, TrumP
    		
    		for (String each : arr) {
    			
    			if(isTitleWord(each)) { 
    				
    				titleFinal += capitalize(each) + " ";
    				
    			} else {
    				
    				titleFinal += each.toLowerCase() + " "; // THE Chronicles OF TrumP --> the Chronicles 
    			}
    			
    		}

    		this.title = titleFinal.substring(0,1).toUpperCase() + titleFinal.substring(1, titleFinal.length()-1);
    	}
